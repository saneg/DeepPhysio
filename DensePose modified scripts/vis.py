...
def vis_one_image(
        ...
        kp_thresh=2, frKey=None, dpi=200, box_alpha=0.0, dataset=None, show_class=False,
        ext='pdf'):
    ...
    #NUOVO CODICE QUESTO EH
    fig2 = plt.figure(num=2, frameon=False)
    ...
    sorted_scores2 = [elem for elem in sorted_scores if boxes[elem, -1] >= thresh]
    
    topScores = []
    for i in range(len(sorted_scores2)):
        topScores.append(boxes[sorted_scores2[i], -1])

    ident = None
    if len(sorted_scores2) > 0:

        areas = []
        for i in range(len(sorted_scores2)):
            areas.append((boxes[sorted_scores2[i], 2] - boxes[sorted_scores2[i], 0]) * (boxes[sorted_scores2[i], 3] - boxes[sorted_scores2[i], 1]))

        sorted_inds = np.argsort(-np.array(areas))

        ident = sorted_scores2[sorted_inds[0]]

    else:
        ident = sorted_scores[0]
    ...
    if show_class:
        ax.text(
            bbox[0], bbox[1] - 2,
            get_class_string(classes[ident], score, dataset),
            fontsize=3,
            family='serif',
            bbox=dict(
    ...
    # show mask
    if segms is not None and len(segms) > ident:
    ...
    # show keypoints
    plt.figure(2)
    if keypoints is not None and len(keypoints) > ident:
        kpArray = [[None for i in range(2)] for j in range(17)]
        ...
        for l in range(len(kp_lines)):
            ...
            if kps[2, i1] > kp_thresh:
                kpArray[i1] = [kps[0, i1], kps[1, i1]]
                plt.plot(
                    kps[0, i1], kps[1, i1], '.', color=colors[l],
                    markersize=3.0, alpha=0.7)

            if kps[2, i2] > kp_thresh:
                kpArray[i2] = [kps[0, i2], kps[1, i2]]
                plt.plot(
    ...
    keyOutput_name = os.path.basename(im_name).split('.')[0] + '_keypoints.png'
    fig2.savefig(os.path.join(output_dir + '/keypoints_images', '{}'.format(keyOutput_name)), dpi=dpi)
    plt.close('all')
